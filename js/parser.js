...
function formatAttackerImpactRanking(data, weights) {
    ...
    const wPeopleMassacred          = w.peopleMassacred          != null ? Number(w.peopleMassacred)          : 0.3;
...
function parseProvinceNewsLine(eventText, dateStr, data, rawLine) {
    ...
    if (fireballM) {
        ...
        if (necroM) data.fireball.peasantsKilled += parseGameInt(necroM[1]);
        return;
    }
    ...
    const tornadoesM = eventText.match(/Tornadoes scour the lands.{0,80}?([\d,]+) acres of buildings/i);
    if (tornadoesM) {
        data.tornadoes.count++;
        data.tornadoes.acresDestroyed += parseGameInt(tornadoesM[1]);
        return;
    }

    // Fool's Gold — "N gold coins have been turned into worthless lead."
    const foolsGoldM = eventText.match(/^([\d,]+) gold coins have been turned into worthless lead/i);
    if (foolsGoldM) {
        data.foolsGold.count++;
        data.foolsGold.goldDestroyed += parseGameInt(foolsGoldM[1]);
        return;
    }
...
function formatProvinceNewsOutput(data) {
    ...
    if (hasSpellImpacts) {
        ...
        if (data.fireball.count > 0)
            out.push(`  Fireball: ${pluralize(data.fireball.count, 'occurrence')} (${formatNumber(data.fireball.peasantsKilled)} peasants killed)`);
        if (data.tornadoes.count > 0)
