...
// ── Shared parsing helpers (Uto-bdna) ─────────────────────────────────────────

/**
 * Parses Province Logs / Province News / Combined text into header + sections.
 */
...
/**
 * Parses Kingdom News text into header + blocks.
 */
function parseKnBlocks(text) {
    const lines = text.split('\n');
    const headerLines = [];
    const blocks = [];
    let currentBlock = null;
...
/**
 * Renders the report header (title + date/range) into the grid.
 */
function renderReportHeader(grid, headerLines, type) {
    let titleLine, metaLine;
    if (type === 'kn') {
        titleLine = headerLines.find(l => l.trim().length > 0);
        metaLine = headerLines.find(l => l.startsWith('Range:'));
        if (metaLine) metaLine = metaLine.replace(/^Range:\s*/, '');
    } else {
        titleLine = headerLines.find(l => l.trim().length > 0 && !l.startsWith('Date') && !l.match(/^\d/));
        metaLine = headerLines.find(l => l.match(/^(Date|May|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Oct|Nov|Dec|\d)/i) || l.includes(' of YR') || l.includes(' - '));
    }
...
// ── Variant B: Compact Table (Uto-bdna) ───────────────────────────────────────

/**
 * Renders Province Logs / Province News / Combined as compact tables.
 */
function renderTableSections(grid, text) {
    const { headerLines, sections } = parseTextIntoSections(text);
    renderReportHeader(grid, headerLines, 'sections');

    for (const section of sections) {
        if (section.lines.every(l => l.trim() === '')) continue;

        const card = document.createElement('div');
        card.className = 'ev-b-card';

        const hdrBar = document.createElement('div');
        hdrBar.className = 'ev-b-header';
        const accentColor = EV_SECTION_COLORS[section.title] || '#555566';
        hdrBar.style.backgroundColor = accentColor;
        hdrBar.textContent = section.title;
        card.appendChild(hdrBar);
        ...
        for (let i = 0; i < end; i++) {
            ...
            const row = document.createElement('div');
            row.className = 'ev-b-row' + (rowIdx % 2 === 1 ? ' ev-b-alt' : '');

            if (line.startsWith('    ')) {
                // Sub-item — indented
                row.classList.add('ev-b-sub');
                row.textContent = line.trimStart();
            } else if (line.startsWith('  ') && line.trimEnd().endsWith(':') && !line.includes('(')) {
                // Sub-section header
                row.classList.add('ev-b-section-label');
...
/**
 * Renders Kingdom News as compact tables.
 */
function renderTableKingdomNews(grid, text) {
    const { headerLines, blocks } = parseKnBlocks(text);
    renderReportHeader(grid, headerLines, 'kn');
    ...
    for (const block of blocks) {
        ...
        const hdrBar = document.createElement('div');
        hdrBar.className = 'ev-b-header';
        if (/^Own Kingdom/.test(block.title)) hdrBar.style.backgroundColor = '#6a9aba';
        else hdrBar.style.backgroundColor = '#555566';
        hdrBar.textContent = block.title;
        card.appendChild(hdrBar);
        ...
        while (end > 0 && block.lines[end - 1].trim() === '') end--;

        let rowIdx = 0;
        for (let i = 0; i < end; i++) {
            const line = block.lines[i];
            if (line.trim() === '') { rowIdx = 0; continue; }
...
// ── Variant C: Dashboard (Uto-bdna) ──────────────────────────────────────────

/**
 * Extracts top-level "Key: Value" items from lines as metric chips,
 * and returns remaining lines as detail items.
 */
function extractMetrics(lines) {
    ...
    for (let i = 0; i < end; i++) {
        const line = lines[i];
        if (line.trim() === '') { details.push(line); continue; }
...
/**
 * Renders Province Logs / Province News / Combined as dashboard cards.
 */
function renderDashboardSections(grid, text) {
    const { headerLines, sections } = parseTextIntoSections(text);
    renderReportHeader(grid, headerLines, 'sections');
    ...
    for (const section of sections) {
        ...
        const card = document.createElement('div');
        card.className = 'ev-c-card';
        const accentColor = EV_SECTION_COLORS[section.title] || '#555566';
        card.style.borderTopColor = accentColor;

        const cardTitle = document.createElement('div');
        cardTitle.className = 'ev-c-title';
        cardTitle.textContent = section.title;
        card.appendChild(cardTitle);

        const { metrics, details } = extractMetrics(section.lines);

        // Metric chips
        if (metrics.length > 0) {
            const chipRow = document.createElement('div');
            chipRow.className = 'ev-c-chips';
            for (const m of metrics) {
                const chip = document.createElement('div');
                chip.className = 'ev-c-chip';
                const val = document.createElement('div');
                val.className = 'ev-c-chip-value';
                val.textContent = m.value;
                const lbl = document.createElement('div');
                lbl.className = 'ev-c-chip-label';
        ...
        // Detail lines in a collapsible section
        const nonEmptyDetails = details.filter(l => l.trim() !== '');
        if (nonEmptyDetails.length > 0) {
            const det = document.createElement('details');
            det.className = 'ev-c-details';
            const sum = document.createElement('summary');
            sum.textContent = 'Details (' + nonEmptyDetails.length + ' items)';
            det.appendChild(sum);

            for (const line of details) {
                if (line.trim() === '') continue;
                const el = document.createElement('div');
                el.className = 'ev-c-detail-item';
                if (line.startsWith('    ')) {
                    el.style.paddingLeft = '1rem';
                    el.textContent = line.trimStart();
                } else if (line.startsWith('  ') && line.trimEnd().endsWith(':') && !line.includes('(')) {
                    el.style.fontWeight = '600';
                    el.style.color = '#7a8290';
...
/**
 * Renders Kingdom News as dashboard cards.
 */
function renderDashboardKingdomNews(grid, text) {
    const { headerLines, blocks } = parseKnBlocks(text);
    renderReportHeader(grid, headerLines, 'kn');
    ...
    for (const block of blocks) {
        ...
        const card = document.createElement('div');
        card.className = 'ev-c-card';
        if (/^Own Kingdom/.test(block.title)) card.style.borderTopColor = '#6a9aba';
        else card.style.borderTopColor = '#555566';
        ...
        const { metrics, details } = extractMetrics(block.lines);
